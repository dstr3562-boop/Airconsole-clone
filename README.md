...
- **🧠 Trivia** — each game randomly samples 10 questions from a pool of
  ~193 general-knowledge questions (world capitals + science, history,
  math, sports, animals, space, food, and more) defined in
  `public/questions.js`. Wrong answers are chosen randomly from other
  real answers in the same category, so options vary between plays too.
- **🏳️ Flags** — same quiz engine, different question bank: 10 random
  countries (from a pool of 60) shown as their flag emoji, with 3 random
  wrong country names as distractors. No image files needed — flags
  render as Unicode emoji.
- **⚡ Speed Tap** — 5 rounds of a pure reaction-time buzzer. Screen
  shows "Wait for green…" then flashes green after a random delay;
  first phone to tap wins the round and 500 points. Tapping before the
  flash doesn't count.

To add more questions or countries, edit the arrays at the top of
`public/questions.js` — everything downstream (shuffling, distractor
generation, scoring) adapts automatically.
